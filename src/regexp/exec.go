...
// machine 保存 p 的 NFA 模拟期间的所有状态。
type machine struct {
	re       *Regexp      // 对应的 Regexp
	p        *syntax.Prog // 编译后的程序
	q0, q1   queue        // 用于 runq、nextq 的两个队列
	pool     []*thread    // 可用线程池
...
// 用于检查零宽标志如 ^ $ \A \z \B \b。
// 它记录相关 rune 对，并且直到绝对必要时才确定隐含的标志
// （大多数时候，这意味着永远不会）。
...
// match 在从 pos 开始的输入上运行机器。
// 它报告是否找到了匹配。
// 如果找到，m.matchcap 保存子匹配信息。
...
// step 执行机器的一步，运行 runq 上的每个线程并将新线程追加到 nextq。
// 该步骤处理 rune c（可能是 endOfText），
// 它从位置 pos 开始并在 nextPos 结束。
// nextCond 给出 c 之后的零宽标志设置。
...
// 它还递归地为通过满足 cond 的零宽条件从 pc 可达的
// 所有指令添加条目。pos 给出输入中的当前位置。
...
// doMatch 报告 r、b 或 s 是否匹配正则表达式。
func (re *Regexp) doMatch(r io.RuneReader, b []byte, s string) bool {
	return re.doExecute(r, b, s, 0, 0, nil) != nil
}

// doExecute 在输入中查找最左匹配，将其子表达式的位置追加到 dstCap 并返回 dstCap。
//
// 如果没有找到匹配则返回 nil，如果找到匹配则返回非 nil。
...
// arrayNoInts 在 ncap=0 时传递 nil dstCap 给 doExecute 匹配时返回。
