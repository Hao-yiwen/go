...
// patchList 是需要填充（修补）的指令指针列表。
// 因为指针还没有被填充，我们可以重用它们的存储来保存列表。
// 这有点投机取巧，但在实践中效果很好。
// 灵感来自 https://swtch.com/~rsc/regexp/regexp1.html。
//
// 这些实际上不是指针：它们是整数，所以我们可以这样重新解释它们
// 而不使用 unsafe 包。值 l.head 表示
// p.inst[l.head>>1].Out（l.head&1==0）或 .Arg（l.head&1==1）。
// head == 0 表示空列表，这没问题因为我们用 fail 指令开始每个程序，
// 所以我们永远不会想要指向它的输出链接。
...
// loop 返回加号或星号主循环的片段。
// 对于加号，可以在将入口更改为 f1.i 后使用。
// 对于星号，当 f1 不能匹配空字符串时可以直接使用。
