...
//
// 磁带归档（tar）是一种用于存储文件序列的文件格式，
// 可以以流的方式进行读写。
// 本包旨在涵盖该格式的大多数变体，
// 包括由 GNU 和 BSD tar 工具生成的格式。
package tar
...
// BUG: 在 32 位架构上使用 Header 中的 Uid 和 Gid 字段可能会溢出。
...
const (
	...
	// 类型 'x' 被 PAX 格式用于存储仅与下一个文件相关的键值记录。
	// 本包透明地处理这些类型。
	TypeXHeader = 'x'

	// 类型 'g' 被 PAX 格式用于存储与所有后续文件相关的键值记录。
	// 本包仅支持解析和组合此类头部，
	// 但目前不支持跨文件持久化全局状态。
	TypeXGlobalHeader = 'g'
	...
	// 类型 'L' 和 'K' 被 GNU 格式用于存储下一个文件的路径或链接名的元文件。
	// 本包透明地处理这些类型。
	TypeGNULongName = 'L'
	TypeGNULongLink = 'K'
)
...
const (
	...
	paxMtime    = "mtime"
	paxAtime    = "atime"
	paxCtime    = "ctime"   // 在 PAX 规范的后续修订版中被移除，但曾经有效
	paxCharset  = "charset" // 当前未使用
	paxComment  = "comment" // 当前未使用

	paxSchilyXattr = "SCHILY.xattr."

	// PAX 扩展头中 GNU 稀疏文件的关键字。
...
// basicKeys 是我们内置支持的 PAX 键集合。
// 不包含 "charset" 或 "comment"，因为它们都是 PAX 特有的，
// 所以不太可能将它们作为 Header 的一等特性添加。
...
// Header 表示 tar 归档中的单个头部。
// 某些字段可能未填充。
//
// 为了向前兼容，从 Reader.Next 获取 Header、
// 以某种方式修改它、然后将其传回 Writer.WriteHeader 的用户
// 应该通过创建新的 Header 并复制他们想要保留的字段来实现。
type Header struct {
	// Typeflag 是头部条目的类型。
	...
	// 如果未指定 Format，则 Writer.WriteHeader 会将 ModTime 四舍五入到最近的秒，
	// 并忽略 AccessTime 和 ChangeTime 字段。
	//
	...
	// Xattrs 将扩展属性作为 PAX 记录存储在 "SCHILY.xattr." 命名空间下。
	//
	// 以下语义等价：
	//  h.Xattrs[key] = value
	//  h.PAXRecords["SCHILY.xattr."+key] = value
	//
	// 当调用 Writer.WriteHeader 时，Xattrs 的内容优先于 PAXRecords 中的内容。
	//
	// 已弃用：请改用 PAXRecords。
	...
	// 用户定义的记录应具有以下形式的键：
	//	VENDOR.keyword
	// 其中 VENDOR 是全大写的某个命名空间，keyword 不能包含 '=' 字符
	// （例如 "GOLANG.pkg.version"）。
	// 键和值应该是非空的 UTF-8 字符串。
	//
	// 当调用 Writer.WriteHeader 时，从 Header 中其他字段派生的 PAX 记录
	// 优先于 PAXRecords。
	PAXRecords map[string]string

	// Format 指定 tar 头部的格式。
	//
	// 这由 Reader.Next 设置，是对格式的最佳猜测。
	// 由于 Reader 会宽松地读取一些不合规的文件，
	// 因此这可能是 FormatUnknown。
	//
	// 如果在调用 Writer.WriteHeader 时未指定格式，
	// 则使用第一个能够编码此 Header 的格式（按 USTAR、PAX、GNU 的顺序）
	// （参见 Format）。
	Format Format
}

// sparseEntry 表示文件中偏移量为 Offset、长度为 Length 的片段。
type sparseEntry struct{ Offset, Length int64 }

func (s sparseEntry) endOffset() int64 { return s.Offset + s.Length }

// 稀疏文件可以表示为 sparseDatas 或 sparseHoles。
// 只要知道总大小，它们就是等价的，可以相互转换。
// 支持稀疏文件的各种 tar 格式以 sparseDatas 形式表示稀疏文件。
// 也就是说，它们指定文件中有数据的片段，并将其他所有内容视为零字节。
// 因此，本包中的编码和解码逻辑处理的是 sparseDatas。
//
// 然而，外部 API 使用 sparseHoles 而不是 sparseDatas，因为
// sparseHoles 的零值逻辑上表示一个普通文件（即其中没有空洞）。
...
//	}
//
// 那么 Header.Size 为 25 的结果稀疏文件的内容是：
//
...
// validateSparseEntries 报告 sp 是否是有效的稀疏映射。
// sp 表示数据片段还是空洞片段都无关紧要。
func validateSparseEntries(sp []sparseEntry, size int64) bool {
	// 验证所有稀疏条目。这些检查与 BSD tar 工具执行的检查相同。
	if size < 0 {
		return false
	}
	var pre sparseEntry
	for _, cur := range sp {
		...
		case cur.Offset < 0 || cur.Length < 0:
			return false // 负值永远不允许
		case cur.Offset > math.MaxInt64-cur.Length:
			return false // 大长度值导致整数溢出
...
// alignSparseEntries 修改 src 并返回 dst，其中每个片段的
// 起始偏移量向上对齐到最近的块边界，
// 结束偏移量向下对齐到最近的块边界。
//
// 尽管 Go tar Reader 和 BSD tar 工具可以处理任意偏移量和长度的条目，
// 但 GNU tar 工具只能处理 blockSize 倍数的偏移量和长度。
...
// 输入必须已经过验证。
//
// 此函数修改 src 并返回规范化的映射，其中：
//   - 相邻片段合并在一起
//   - 只有最后一个片段可以为空
//   - 最后一个片段的 endOffset 是总大小
...
// fileState 跟踪当前文件剩余的逻辑字节数（包括稀疏空洞）
// 和物理字节数（tar 归档中的实际字节数）。
//
...
// 返回值是多个可能格式的逻辑或。
// 如果值是 FormatUnknown，则无法编码输入的 Header，
// 并返回解释原因的错误。
//
// 作为检查字段的副产品，此函数返回 paxHdrs，
// 其中包含所有无法直接编码的字段。
...
func (h Header) allowedFormats() (format Format, paxHdrs map[string]string, err error) {
	...
	var preferPAX bool // 优先使用 PAX 而不是 USTAR
	verifyString := func(s string, size int, name, paxKey string) {
		// 对于 path 和 linkpath，NUL 终止符是可选的。
		// 技术上，uname 和 gname 需要它，
	...
	*/
	...
	if wantFormat := h.Format; wantFormat != FormatUnknown {
		if wantFormat.has(FormatPAX) && !preferPAX {
			wantFormat.mayBe(FormatUSTAR) // PAX 意味着也允许 USTAR
		}
		format.mayOnlyBe(wantFormat) // 设置允许格式和期望格式的并集
	}
...
const (
	...
	// 通用 Unix 模式常量；这些未在任何通用 tar 标准中定义。
	// Header.FileInfo 理解这些，但 FileInfoHeader 永远不会生成这些。
	c_ISDIR  = 040000  // 目录
...
// 如果 fi 描述的是符号链接，FileInfoHeader 将 link 记录为链接目标。
// 如果 fi 描述的是目录，则在名称后附加斜杠。
//
// 由于 fs.FileInfo 的 Name 方法只返回其描述的文件的基本名称，
// 可能需要修改 Header.Name 以提供文件的完整路径名。
//
// 如果 fi 实现了 [FileInfoNames]，
// Header.Gname 和 Header.Uname
// 由该接口的方法提供。
func FileInfoHeader(fi fs.FileInfo, link string) (*Header, error) {
	...
	// 如果可能，从操作系统特定的 FileInfo 字段填充附加字段。
	if sys, ok := fi.Sys().(*Header); ok {
		// 此 FileInfo 来自 Header（而非操作系统）。使用
		// 原始 Header 填充所有剩余字段。
		h.Uid = sys.Uid
		h.Gid = sys.Gid
		h.Uname = sys.Uname
...
// FileInfoNames 扩展 [fs.FileInfo]。
// 将此接口的实例传递给 [FileInfoHeader] 允许调用者
// 通过直接指定 Uname 和 Gname 来避免系统相关的名称查找。
...
// isHeaderOnlyType 检查给定的类型标志是否属于即使指定了大小也没有数据段的类型。
