...
// Use of this source code is governed by a BSD-style
// license that can be found in the LICENSE file.`+"\n\n")
	fmt.Fprintf(&buf, "// 代码由 go run makeisprint.go -output isprint.go 生成；勿需编辑。\n\n")
	fmt.Fprintf(&buf, "package strconv\n\n")

	fmt.Fprintf(&buf, "// (%d+%d+%d)*2 + (%d)*4 = %d 字节\n\n",
	...
	fmt.Fprintf(&buf, "var isNotPrint32 = []uint16{ // 每个条目加 0x10000\n")
	for _, r := range except32 {
		if r >= 0x20000 {
			log.Fatalf("%U 对于 isNotPrint32 太大\n", r)
		}
		fmt.Fprintf(&buf, "\t%#04x,\n", r-0x10000)
	}
	fmt.Fprintf(&buf, "}\n\n")

	// 图形但非"可打印" rune 的列表很短。只需制作一个简单的表格。
	fmt.Fprintf(&buf, "// isGraphic 列出不被 IsPrint 匹配的图形 rune。\n")
	fmt.Fprintf(&buf, "var isGraphic = []uint16{\n")
	for r := rune(0); r <= unicode.MaxRune; r++ {
		if unicode.IsPrint(r) != unicode.IsGraphic(r) {
			// 完整性检查。
			if !unicode.IsGraphic(r) {
				log.Fatalf("%U 是可打印但不是图形\n", r)
			}
			if r > 0xFFFF { // 我们只期望 16 位值。
				log.Fatalf("%U 对于 isGraphic 太大\n", r)
			}
			fmt.Fprintf(&buf, "\t%#04x,\n", r)
		}
	}
