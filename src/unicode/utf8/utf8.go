...
// 它包含在 rune 和 UTF-8 字节序列之间转换的函数。
// 参见 https://en.wikipedia.org/wiki/UTF-8
package utf8

// 条件 RuneError==unicode.ReplacementChar 和
// MaxRune==unicode.MaxRune 在测试中已验证。
// 在本地定义它们可避免此包依赖 unicode 包。

// 编码的基本数字。
const (
	RuneError = '\uFFFD'     // "错误" Rune 或 "Unicode 替换字符"
	RuneSelf  = 0x80         // 低于 RuneSelf 的字符用单个字节表示自身。
...
const (
	...
	// 这些常量的名称被选择为在下表中提供良好的对齐。
	// 第一个半字节是 acceptRanges 的索引，或 F 表示特殊的单字节情况。
	// 第二个半字节是 Rune 长度或特殊单字节情况的状态。
	xx = 0xF1 // 无效：大小 1
	as = 0xF0 // ASCII：大小 1
	s1 = 0x02 // 接受 0，大小 2
	s2 = 0x13 // 接受 1，大小 3
...
// acceptRanges 大小为 16 以避免使用它的代码中的边界检查。
...
// FullRune 报告 p 中的字节是否以完整的 rune UTF-8 编码开始。
// 无效编码被视为完整的 Rune，因为它将转换为宽度为 1 的错误 rune。
...
// FullRuneInString 类似于 FullRune，但其输入是字符串。
...
// DecodeRune 解包 p 中的第一个 UTF-8 编码并返回 rune 及其字节宽度。
// 如果 p 为空，返回 ([RuneError], 0)。否则，如果编码无效，
// 返回 (RuneError, 1)。对于正确的非空 UTF-8，这两种结果都是不可能的。
//
// 如果编码是不正确的 UTF-8、编码的 rune 超出范围、
// 或不是该值的最短可能 UTF-8 编码，则编码无效。
// 不执行其他验证。
...
// DecodeRuneInString 类似于 [DecodeRune]，但其输入是字符串。如果 s 为空，
// 返回 ([RuneError], 0)。否则，如果编码无效，返回 (RuneError, 1)。
...
// DecodeLastRune 解包 p 中的最后一个 UTF-8 编码并返回 rune 及其字节宽度。
// 如果 p 为空，返回 ([RuneError], 0)。否则，如果编码无效，
// 返回 (RuneError, 1)。对于正确的非空 UTF-8，这两种结果都是不可能的。
//
// 如果编码是不正确的 UTF-8、编码的 rune 超出范围、
...
// DecodeLastRuneInString 类似于 [DecodeLastRune]，但其输入是字符串。
// 如果 s 为空，返回 ([RuneError], 0)。否则，如果编码无效，
// 返回 (RuneError, 1)。对于正确的非空 UTF-8，这两种结果都是不可能的。
//
// 如果编码是不正确的 UTF-8、编码的 rune 超出范围、
// 或不是该值的最短可能 UTF-8 编码，则编码无效。
...
// RuneLen 返回 rune 的 UTF-8 编码中的字节数。
// 如果 rune 不是有效的 UTF-8 编码值，则返回 -1。
...
// 如果 rune 超出范围，它会写入 [RuneError] 的编码。
// 它返回写入的字节数。
...
// 如果 rune 超出范围，它会追加 [RuneError] 的编码。
...
// RuneCount 返回 p 中的 rune 数量。错误和短编码
// 被视为宽度为 1 字节的单个 rune。
...
// RuneCountInString 类似于 [RuneCount]，但其输入是字符串。
...
// RuneStart 报告该字节是否可能是已编码的（可能无效的）rune 的第一个字节。
// 第二个及后续字节的高两位始终设置为 10。
...
func Valid(p []byte) bool {
	// 此优化避免了在生成切片 p 的代码时需要重新计算容量，
	// 使其与 ValidString 达到同等水平，
	// 后者在长 ASCII 字符串上快 20%。
	p = p[:len(p):len(p)]
