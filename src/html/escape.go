...
// 这些替换允许与假定使用 Windows-1252 编码的旧数字实体兼容。
// https://html.spec.whatwg.org/multipage/parsing.html#numeric-character-reference-end-state
var replacementTable = [...]rune{
	'\u20AC', // 第一个条目是 0x80 应被替换成的字符。
	'\u0081',
	'\u201A',
	'\u0192',
	'\u201E',
...
// unescapeEntity 从 b[src:] 读取类似 "&lt;" 的实体，并将对应的
// "<" 写入 b[dst:]，返回递增后的 dst 和 src 游标。
// 前置条件：b[src] == '&' && dst <= src。
func unescapeEntity(b []byte, dst, src int, entity map[string]rune, entity2 map[string][2]rune) (dst1, src1 int) {
	...
	if s[i] == '#' {
		...
		if 0x80 <= x && x <= 0x9F {
			// 将 Windows-1252 字符替换为 UTF-8 等价字符。
...
var htmlEscaper = strings.NewReplacer(
	`&`, "&amp;",
	`'`, "&#39;", // "&#39;" 比 "&apos;" 短，而且 apos 直到 HTML5 才出现在 HTML 中。
	`<`, "&lt;",
	`>`, "&gt;",
	`"`, "&#34;", // "&#34;" 比 "&quot;" 短。
)

// EscapeString 将特殊字符如 "<" 转义为 "&lt;"。
// 它只转义五个这样的字符：<、>、&、' 和 "。
// [UnescapeString](EscapeString(s)) == s 始终成立，但反过来不一定成立。
func EscapeString(s string) string {
	return htmlEscaper.Replace(s)
}

// UnescapeString 将类似 "&lt;" 的实体反转义为 "<"。它能反转义比
// [EscapeString] 转义范围更广的实体。例如，"&aacute;"
// 会被反转义为 "á"，"&#225;" 和 "&#xE1;" 也是如此。
// UnescapeString([EscapeString](s)) == s 始终成立，但反过来不一定成立。
