...
// 一个Directive 是一个 comment of this form:
//
//	//tool:name args
//
...
// -trimprefix Op".
//
// While Args does not have a strict syntax, by convention it 是一个
...
type Directive struct {
	...
	// Slash 是 position of the "//" at the beginning of the directive.
	Slash token.Pos

	// ArgsPos 是 position where Args begins, based on the position passed
	// to ParseDirective.
	ArgsPos token.Pos
}
...
//
// The provided text 必须是 a single line and should include the leading "//".
// If the text does not start with "//", it returns false.
//
// The caller may provide a file position of the start of c. This 将是 used
// to track the position of the arguments. This 可能是 [Comment.Slash],
...
// 一个DirectiveArg 是一个n argument to a directive comment.
type DirectiveArg struct {
	// Arg 是 parsed argument string. If the argument was a quoted string,
	// this is its unquoted form.
	Arg string
	// Pos 是 position of the first character in th是一个rgument.
	Pos token.Pos
}

// ParseArgs parses a [Directive]'s arguments using the standard convention,
// which 是一个 sequence of tokens, where each token 可能是 a bare word, or a
// double quoted Go string, or a back quoted raw Go string. Each token must be
// separated by one or more Unicode spaces.
//
// If the arguments do not conform to this syntax, it 返回一个 error.
...
// directiveScanner 是一个 helper for parsing directive comments while maintaining
