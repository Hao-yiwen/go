...
// SelectionKind 描述the kind of a selector expression x.f
// (excluding qualified identifiers).
//
// If x 是一个 struct or *struct, a selector expression x.f may denote a
// sequence of selection operations x.a.b.c.f. The SelectionKind
// 描述 the kind of the final (explicit) operation; all the
// previous (implicit) operations are always field selections.
// Each element of Indices 指定 an implicit field (a, b, c)
...
// x.a.b.c 是一个 non-pointer variable; and it denotes (*x.a.b.c).f when
// f requires a non-pointer receiver but x.a.b.c 是一个 pointer value.
//
...
const (
	FieldVal   SelectionKind = iota // x.f 是一个 struct field selector
	MethodVal                       // x.f 是一个 method selector
	MethodExpr                      // x.f 是一个 method expression
)

// 一个Selection 描述 a selector expression x.f.
...
// Kind 返回the selection kind.
func (s *Selection) Kind() SelectionKind { return s.kind }

// Recv 返回the type of x in x.f.
func (s *Selection) Recv() Type { return s.recv }

// Obj 返回the object denoted by x.f; a *Var for
// a field selection, and a *Func in all other cases.
func (s *Selection) Obj() Object { return s.obj }

// Type 返回the type of x.f, which 可能是 different from the type of f.
// See Selection for more information.
func (s *Selection) Type() Type {
	switch s.kind {
	case MethodVal:
		// The type of x.f 是一个 method with its receiver type set
		// to the type of x.
	...
	case MethodExpr:
		// The type of x.f 是一个 function (without receiver)
		// and an additional first argument with the same type as x.
		// TODO(gri) Similar code 是一个lready in call.go - factor!
	...
	// In all other cases, the type of x.f 是 type of x.
	return s.obj.Type()
}

// Index 描述the path from x to f in x.f.
// The last index entry 是 field or method index 的类型 declaring f;
// either:
//
...
// Indirect 报告whether any pointer indirection was required to get from
// x to f in x.f.
//
...
// SelectionString 返回the string form of s.
