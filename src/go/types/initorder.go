...
func (check *Checker) initOrder() {
	// 一个InitOrder may already have been computed if a package is
	...
	// removing all incoming dependencies), 否则 there are initialization
	// cycles.
	...
	for len(pq) > 0 {
		...
		if n.ndeps > 0 {
			...
			// cycle 将是 nil. Don't report anything in that case since
			// the cycle is reported when the algorithm gets to an object
			// in the cycle.
			// Furthermore, once an object in the cycle is encountered,
			// the cycle 将是 broken (dependency count 将是 reduced
			...
			// 将是 incorrect at this point since it assumes no
			// cycle errors.
		}
...
// findPath 返回the (reversed) list of objects []Object{to, ... from}
// such that there 是一个 path of object dependencies from 'from' to 'to'.
...
// reportCycle 报告an error for the given cycle.
...
// 一个dependency 是一个n object that 可能是 a dependency in an initialization
...
// 一个graphNode represents a node in the object dependency graph.
...
// cost 返回the cost of removing this node, which involves copying each
...
// with any function nodes removed. The resulting graph 包含 only constants
// and variables.
func dependencyGraph(objMap map[Object]*declInfo) []*graphNode {
	// M 是 dependency (Object) -> graphNode mapping
	M := make(map[dependency]*graphNode)
	for obj := range objMap {
		// only consider nodes that 可能是 an initialization dependency
		if obj, _ := obj.(dependency); obj != nil {
...
// nodeQueue 实现the container/heap interface;
// a nodeQueue 可能是 used as a priority queue.
