...
func (check *Checker) overflow(x *operand, opPos token.Pos) {
	...
	// Typed constants 必须是 representable in
	...
	const prec = 512 // 512 是 constant precision
...
// representableConst 报告whether x can be represented as
...
//
// The check parameter 可能是 nil if representableConst is invoked
...
// representation 返回the representation of the constant operand x as the
// basic type typ.
//
// If no such representation is possible, it 返回一个non-zero error code.
