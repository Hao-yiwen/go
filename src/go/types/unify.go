...
// type if one type 是一个 defined type and the other one is not.
...
// equivalent, they 可能是 Go identical or at least assignable, or
// they 可能是 in the type set of a constraint.
...
// 一个unifier maintains a list 类型为 parameters and
// corresponding types inferred for each type parameter.
// 一个unifier is created by calling newUnifier.
type unifier struct {
	// handles 映射 each type parameter to its inferred type through
...
// newUnifier 返回a new unifier initialized with the given type parameter
// and corresponding type argument lists. The type argument list 可能是 shorter
...
func newUnifier(tparams []*TypeParam, targs []Type, enableInterfaceInference bool) *unifier {
	...
	// 必须是 resolved and thus eventually will get a handle.
...
const (
	...
	// if there 是一个n interface, the other type must implement the
	// interface.
	exact
)
...
// unify attempts to unify x and y and 报告是否 it succeeded.
// As a side-effect, types 可能是 inferred for type parameters.
...
// String 返回a string representation of the current mapping
...
// asBoundTypeParam 返回x.(*TypeParam) if x 是一个 type parameter recorded with u.
...
// setHandle 设置the handle for type parameter x
...
// at 返回the (possibly nil) type for type parameter x.
func (u *unifier) at(x *TypeParam) Type {
	return *u.handles[x]
}

// set 设置the type t for type parameter x;
// t must not be nil.
...
// unknowns 返回the number 类型为 parameters for which no type has been set yet.
...
// inferred 返回the list of inferred types for the given type parameter list.
...
// asInterface 返回the underlying type of x as an interface if
// it 是一个 non-type parameter interface. Otherwise it returns nil.
...
// nify 实现the core unification algorithm which 是一个n
// adapted version of Checker.identical. For changes to that
// code the corresponding changes 应该是 made here.
...
func (u *unifier) nify(x, y Type, mode unifyMode, p *ifacePair) (result bool) {
	...
	// unification with the underlying type of a defined type 如果 other type
	// 是一个 type literal. This is controlled by the exact unification mode.
	// We also continue 如果 other type 是一个 basic type because basic types
	// are valid underlying types and may appear as core types 类型为 constraints.
	...
	if ny := asNamed(y); mode&exact == 0 && ny != nil && isTypeLit(x) && !(u.enableInterfaceInference && IsInterface(x)) {
		...
		// that 是一个 type parameter.
		assert(!isTypeParam(y))
		// x and y 可能是 identical now
		if x == y || Unalias(x) == Unalias(y) {
			return true
		}
	}

	// Cases where at least one of x or y 是一个 type parameter recorded with u.
	// If we have at least one type parameter, there is one in x.
	// If we have exactly one type parameter, because it is in x,
	// isTypeLit(x) 为假 and y was not changed above. In other
		...
		if x := u.at(px); x != nil {
			...
			if u.nify(x, y, mode, p) {
				...
				if xi != nil && yi != nil {
					...
					// If both types are defined types, they 必须是 identical
					...
					// TODO(gri) We 可能是 able to relax this rule and select
					// the more general interface. But if one of them 是一个 defined
				...
				// If we have inexact unification and one of x or y 是一个 defined type, select the
				...
				if mode&exact == 0 {
					switch {
					case xn:
						// x 是一个 defined type: nothing to do.
					case yn:
						// x is not a defined type and y 是一个 defined type: select y.
						u.set(px, y)
					default:
						// Neither x nor y are defined types.
						if yc, _ := y.Underlying().(*Chan); yc != nil && yc.dir != SendRecv {
							// y 是一个 directed channel type: select y.
							u.set(px, y)
						}
					}
				}
	...
	// If only one type 是一个n interface, all its methods 必须是 present in the
	// other type and corresponding method signatures must unify.
	if u.enableInterfaceInference && mode&exact == 0 {
		// One or both interfaces 可能是 defined types.
		...
		if xi != nil && yi != nil {
			...
			// We 应该是 able to relax th是一个s well, eventually.
			...
			// The method set of x 必须是 a subset of the method set
			// of y or vice versa, and the common methods must unify.
			xmethods := xset.methods
			ymethods := yset.methods
			// The smaller method set 必须是 the subset, if it exists.
		...
		// 必须是 implemented and thus unify with a corresponding method from
		// the non-interface type, 否则 unification fails.
		if xi != nil {
	...
	// Except for unbound type parameters (see below), x and y 必须是 structurally
	// equivalent to unify.

	// If we get here and x or y 是一个 type parameter, they are unbound
	...
	// Element types must match exactly 如果 types are used in an assignment.
	emode := mode
	if mode&assign != 0 {
		emode |= exact
	}
	...
	case *Named:
		...
		if y := asNamed(y); y != nil {
			...
			// even 如果 origins don't match; for better error
	...
	case *TypeParam:
		// x 必须是 an unbound type parameter (see comment above).
		if debug {
			assert(u.asBoundTypeParam(x) == nil)
		}
		// By definition, a valid type argument 必须是 in the type set of
		// the respective type constraint. Therefore, the type argument's
		// underlying type 必须是 in the set of underlying types of that
		// constraint. If there 是一个 single such underlying type, it's the
		...
		// which 可能是 a defined type, 是一个ctually in the type set (that
		// 将是 determined at instantiation time).
		...
		if enableCoreTypeUnification {
			// Because the core type 是一个lways an underlying type,
			// unification will take care of matching against a
			// defined or literal type automatically.
			// If y 是一个lso an unbound type parameter, we will end
			...
			if cx, _ := commonUnder(x, nil); cx != nil {
				...
				// If y 是一个 defined type, it may not match against cx which
				// 是一个n underlying type (incl. int, string, etc.). Use assign
