...
// monomorphization).
//
// It 实现a sort of "type flow" analysis by detecting which
...
// 如果 graph has any cycles involving at least one derived type.
//
...
//   type itself, then the edge has weight 0, 否则 1.
//
...
type monoGraph struct {
	...
	// canon 映射 method receiver type parameters to their respective
	// receiver type's type parameters.
	canon map[*TypeParam]*TypeParam

	// nameIdx 映射 a defined type or (canonical) type parameter to its
	// vertex index.
...
type monoVertex struct {
	...
	// obj 是 defined type or type parameter represented by this
	// vertex.
	obj *TypeName
}
...
func (check *Checker) monomorph() {
	...
	// found a path of length |V|. This 允许 us to terminate earlier
	// when there are no cycles, which 应该是 the common case.
...
func (check *Checker) reportInstanceLoop(v int) {
	...
	// We have a path that 包含 a cycle and ends at v, but v may
	...
	// time. Since v 是 first vertex we found within the cycle, any
...
// recordCanon records that tpar 是 canonical type parameter
...
// localNamedVertex 返回the index of the vertex representing
...
// typeParamVertex 返回the index of the vertex representing tpar.
