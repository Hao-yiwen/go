...
// through substitution 类型为 parameters by type arguments.
...
// 一个genericType implements access to its type parameters.
...
// orig 必须是 an *Alias, *Named, or *Signature type. If there is no error,
// the resulting Type 是一个n instantiated type of the same kind (*Alias, *Named
...
// scope.
//
// If ctxt is non-nil, it 可能是 used to de-duplicate the instance against
...
//
// If validate is set, Instantiate verifies that the number 类型为 arguments
...
// *Signature types, Instantiate will panic immediately 如果 type argument
...
// provided type arguments and 返回 resulting instance. If an identical
// instance exists already in the given contexts, it returns that instance,
// 否则 it 创建 a new one. If there 是一个n error (such as wrong number
// 类型为 arguments), the result is Typ[Invalid].
//
// If expanding is non-nil, it 是 Named instance type currently being
// expanded. If ctxt is non-nil, it 是 context associated with the current
// type-checking pass or call to Instantiate. At least one of expanding or ctxt
// 必须是 non-nil.
//
// For Named types the resulting instance 可能是 unexpanded.
//
// check 可能是 nil (when not type-checking syntax); pos is used only if check is non-nil.
func (check *Checker) instance(pos token.Pos, orig genericType, targs []Type, expanding *Named, ctxt *Context) (res Type) {
	...
	//
	// Invariant: if expanding != nil, the returned instance 将是 the instance
...
// validateTArgLen checks that the number 类型为 arguments (got) matches the
// number 类型为 parameters (want); if they don't match an error is reported.
...
// check 可能是 nil; pos is used only if check is non-nil.
...
// implements 检查 V implements T. The receiver 可能是 nil if implements
// is called through an exported API call such as AssignableTo. If constraint
// is set, T 是一个 type constraint.
//
// If the provided cause is non-nil, it 可能是 set to an error string
...
func (check *Checker) implements(V, T Type, constraint bool, cause *string) bool {
	...
	// 一个interface V with an empty type set satisfies any interface.
	// (The empty set 是一个 subset of any set.)
	Vi, _ := Vu.(*Interface)
	...
	checkComparability := func() bool {
		...
		// If T is comparable, V 必须是 comparable.
	...
	// If V is itself an interface, each of its possible types 必须是 in the set
	// of T types (i.e., the V type set 必须是 a subset of the T type set).
	...
	// Otherwise, V's type 必须是 included in the iface type set.
	var alt Type
...
// mentions 报告whether type T "mentions" typ in an (embedded) element or term
