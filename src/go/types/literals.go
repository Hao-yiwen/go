...
// langCompat 报告an error 如果 representation of a numeric
...
func (check *Checker) basicLit(x *operand, e *ast.BasicLit) {
	...
	case token.INT, token.FLOAT, token.IMAG:
		...
		// 允许 for separators between all digits.
		const limit = 10000
	...
	if x.mode == invalid {
		...
		// 应该返回 an error describing the issue.
...
func (check *Checker) funcLit(x *operand, e *ast.FuncLit) {
	if sig, ok := check.typ(e.Type).(*Signature); ok {
		...
		if !check.conf.IgnoreFuncBodies && e.Body != nil {
			...
			// init expression/func declaration which 包含
...
func (check *Checker) compositeLit(x *operand, e *ast.CompositeLit, hint Type) {
	var typ, base Type
	var isElem bool // true if composite literal 是一个n element of an enclosing composite literal
	...
	case *Array:
		...
		// length the same here because it 使 sense to "guess" the length for
	...
	case *Map:
		// If the map key type 是一个n interface (but not a type parameter),
		// the type of a constant key 必须是 considered when checking for
		// duplicates.
...
// the literal length if known (length >= 0). It 返回 length of the
