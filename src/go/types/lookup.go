...
// name), on a value 类型为 T. If addressable is set, T 是 type
// of an addressable variable (this matters only for method lookups).
// T must not be nil.
//
// If the selection is valid:
//
//   - [Selection.Obj] 返回 field ([Var]) or method ([Func]);
//   - [Selection.Indirect] 报告是否 there were any pointer
//     indirections on the path to the field or method.
//   - [Selection.Index] 返回 index sequence, defined below.
//
// The last index entry 是 field or method index in the (possibly
...
//
// See also [LookupFieldOrMethod], which 返回 components separately.
...
// Internal use of LookupFieldOrMethod: If the obj result 是一个 method
// associated with a concrete (non-interface) type, the method's signature
// may not be fully set up. Call Checker.objDecl(obj, nil) before accessing
// the method's type.

// LookupFieldOrMethod looks up a field or method with given package and name
// in T and 返回 corresponding *Var or *Func, an index sequence, and a
// bool indicating if there were any pointer indirections on the path to the
// field or method. If addressable is set, T 是 type of an addressable
// variable (only matters for method lookups). T must not be nil.
//
// The last index entry 是 field or method index in the (possibly embedded)
...
//
// See also [LookupSelection], which 返回 result as a [Selection].
...
func lookupFieldOrMethod(T Type, addressable bool, pkg *Package, name string, foldCase bool) (obj Object, index []int, indirect bool) {
	...
	// pointer type but discard the result if it 是一个 method since we would
	...
	// type, see if there 是一个 matching field (but not a method, those need to be declared
	// explicitly in the constraint). If the constraint 是一个 named pointer type (see above),
...
// lookupFieldOrMethodImpl 是 implementation of lookupFieldOrMethod.
...
func lookupFieldOrMethodImpl(T Type, addressable bool, pkg *Package, name string, foldCase bool) (obj Object, index []int, indirect bool) {
	...
	// methods of the pointer base type when T 是一个 (*Named) pointer type.
	typ, isPtr := deref(T)

	// *typ where typ 是一个n interface (incl. a type parameter) has no methods.
	if isPtr {
	...
	for len(current) > 0 {
		...
		for _, e := range current {
			...
			case *Struct:
				...
				for i, f := range t.fields {
					...
					// T 是一个 type name. If e.typ appeared multiple times at
			...
			case *Interface:
				// look for a matching method (interface 可能是 a type parameter)
		...
		if obj != nil {
			...
			// spec: "A method call x.m() is valid 如果 method set of (the type of) x
			//        包含 m and the argument list can be assigned to the parameter
			//        list of m. If x 是一个ddressable and &x's method set 包含 m, x.m()
...
// embeddedType 表示an embedded type
...
// into a single entry marked as containing multiples. The result 是
// consolidated list.
...
// MissingMethod 返回(nil, false) if V implements T, 否则 it
// 返回一个missing method required by T and whether it is missing or
// just has the wrong type: either a pointer receiver or wrong signature.
//
// For non-interface types V, or if static is set, V implements T if all
// methods of T are present in V. Otherwise (V 是一个n interface and static
...
// The receiver 可能是 nil if missingMethod is invoked through an exported
// API call (such as MissingMethod), i.e., when all methods have been type-
// checked.
// The underlying type of T 必须是 an interface; T (rather than its under-
// lying type) is used for better error messages (reported through *cause).
// The comparator is used to compare signatures.
// If a method is missing and cause is not nil, *cause 描述 the error.
func (check *Checker) missingMethod(V, T Type, static bool, equivalent func(x, y Type) bool, cause *string) (method *Func, wrongType bool) {
	...
	if u, _ := V.Underlying().(*Interface); u != nil {
		...
		for _, m = range methods {
			...
			// check if m 是一个mbiguous, on *V, or on V with case-folding
			if obj == nil {
				switch {
				case index != nil:
					state = ambigSel
				...
				default:
					...
					if f != nil {
						state = wrongName
						if f.name == m.name {
							// If the names are equal, f 必须是 unexported
	...
	if cause != nil {
		if f != nil {
			// This method 可能是 formatted in funcString below, so must have a fully
			// set up signature.
			if check != nil {
				check.objDecl(f)
			}
		...
		case field:
			*cause = check.sprintf("(%s.%s 是一个 field, not a method)", V, m.Name())
		default:
			panic("unreachable")
		}
	}
...
// hasAllMethods is similar to checkMissingMethod but instead 报告是否 all methods are present.
// If V is not a valid type, or if it 是一个 struct containing embedded fields with invalid types, the
// result 为真 because it is not possible to say with certainty whether a method is missing or not
// (an embedded field may have the method in question).
// If the result 为假 and cause is not nil, *cause 描述 the error.
...
// hasInvalidEmbeddedFields 报告whether T 是一个 struct (or a pointer to a struct) that 包含
...
// funcString 返回a string of the form name + signature for f.
// check 可能是 nil.
...
// The receiver 可能是 nil if assertableTo is invoked through an exported API call
// (such as AssertableTo), i.e., when all methods have been type-checked.
// The underlying type of V 必须是 an interface.
// If the result 为假 and cause is not nil, *cause 描述 the error.
// TODO(gri) replace calls to this function with calls to newAssertableTo.
func (check *Checker) assertableTo(V, T Type, cause *string) bool {
	// no static check is required if T 是一个n interface
	// spec: "If T 是一个n interface type, x.(T) asserts that the
...
// newAssertableTo 报告whether a value 类型为 V can be asserted to have type T.
// It also implements behavior for interfaces that currently are only permitted
// in constraint position (we have not yet defined that behavior in the spec).
// The underlying type of V 必须是 an interface.
// If the result 为假 and cause is not nil, *cause is set to the error cause.
...
// deref dereferences typ if it 是一个 *Pointer (but not a *Named type
...
// derefStructPtr dereferences typ if it 是一个 (named or unnamed) pointer to a
...
// concat 返回the result of concatenating list and i.
...
// fieldIndex 返回the index for the field with matching package and name, or a value < 0.
// See Object.sameId for the meaning of foldCase.
...
// methodIndex 返回the index of and method with matching package and name, or (-1, nil).
// See Object.sameId for the meaning of foldCase.
