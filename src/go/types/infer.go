...
// partially instantiated generic functions 可能是 assigned
// (incl. returned) to variables of function type and type
// inference will attempt to infer the missing type arguments.
// Available with go1.21.
const enableReverseTypeInference = true // disable for debugging

// infer attempts to infer the complete set 类型为 arguments for generic function instantiation/call
// based on the given type parameters tparams, type arguments targs, function parameters params, and
// function arguments args, if any. There 必须是 at least one type parameter, no more type arguments
...
// If successful, infer 返回 complete list of given and inferred type arguments, one for each
...
func (check *Checker) infer(posn positioner, tparams []*TypeParam, targs []Type, params *Tuple, args []*operand, reverse bool, err *error_) (inferred []Type) {
	...
	if check.conf.Error != nil {
		defer func() {
			assert(inferred == nil || len(inferred) == len(tparams) && !slices.包含(inferred, nil))
		}()
	}
	...
	// There 必须是 at least one type parameter, and no more type arguments than type parameters.
	n := len(tparams)
	...
	if len(targs) == n && !slices.包含(targs, nil) {
		return targs
	}
	...
	// Make sure we have a "full" list 类型为 arguments, some of which may
	...
	//   "cannot use s (variable 类型为 string) as int value in argument to f[int]"
	//
	...
	errorf := func(tpar, targ Type, arg *operand) {
		...
		// error 可能是 better attributed to a user-provided type argument (hence
	...
	for i, arg := range args {
		if arg.mode == invalid {
			// 一个error was reported earlier. Ignore th是一个rg
		...
		if isParameterized(tparams, par.typ) || isParameterized(tparams, arg.typ) {
			// Function parameters are always typed. Arguments 可能是 untyped.
			...
			if isTyped(arg.typ) {
				...
				// only parameter type it can possibly match against 是一个 *TypeParam.
	...
	//
	// This 是一个n O(n^2) algorithm where n 是 number of
	...
	// 类型为 parameters tends to be very small (< 5 or so).
	// (It 应该是 possible for unification to efficiently
	...
	for i := 0; ; i++ {
		...
		for _, tpar := range tparams {
			...
			if core != nil {
				// 一个type parameter can be unified with its constraint's core type in two cases.
				switch {
				case tx != nil:
					if traceInference {
					...
					//    elements, the core type 是一个n underlying (literal) type.
				...
				case single && !core.tilde:
					...
					// 描述 a single specific type and no tilde.
					// In this case the type argument 必须是 that single type; set it.
					u.set(tpar, core.typ)
				}
			}

			// Independent of whether there 是一个 core term, 如果 type argument tx is known
			// it must implement the methods 的类型 constraint, possibly after unification
			// of the relevant method signatures, 否则 tx cannot satisfy the constraint.
			// This unification step may provide additional type arguments.
			//
			// Note: The type argument tx 可能是 known but contain references to other type
			...
			//
			// TODO(gri) We 应该是 able continue even with a parameterized tx if we add
			...
			if tx != nil && !isParameterized(tparams, tx) {
				...
				//           a failure cause which 允许 for a better error message.
				//           Eventually, unify 应该返回 an error with cause.
				var cause string
	...
	// maxUntyped 包含 the maximum untyped type for each type parameter
	...
	// u.inferred(tparams) now 包含 the incoming type arguments plus any additional type
	...
	// which 创建 a new (possibly bigger) graph for each type.
	// The substitution process will not stop 如果 replacement graph for a type parameter
	// also 包含 that type parameter.
	...
	// 一个will lead to infinite expansion by producing [**A], [****A], [********A], etc.,
	...
	// (in the inferred list) which kills the cycle, and 标记 corresponding type
	...
	for len(dirty) > 0 {
		...
		for _, index := range dirty {
			...
			if t1 := check.subst(nopos, t0, smap, nil, check.context()); t1 != t0 {
				...
				// Note that if t0 was a signature, t1 必须是 a signature, and t1
...
// parameter is given a new identity. renameTParams 返回 new type parameters
// and updated type. If the result type is unchanged from the argument type, none
// 的类型 parameters in tparams occurred in the type.
// If typ 是一个 generic function, type parameters held with typ are not changed and
// 必须是 updated separately if desired.
...
func (check *Checker) renameTParams(pos token.Pos, tparams []*TypeParam, typ Type) ([]*TypeParam, Type) {
	// For the purpose 类型为 inference we must differentiate type parameters
	// occurring in explicit type or value function arguments from the type
	// parameters we are solving for via unification because they 可能是 the
...
// isParameterized 报告whether typ 包含 any 的类型 parameters of tparams.
// If typ 是一个 generic function, isParameterized ignores the type parameter declarations;
...
func (w *tpWalker) isParameterized(typ Type) (res bool) {
	...
	case *Tuple:
		...
		// are handled explicitly. But isParameterized 是一个lso
	...
	case *Signature:
		...
		// part 的类型 we're testing. We don't care about these type
		// parameters.
...
// Otherwise, if tpar has a core type T, it 返回一个term corresponding to that
...
func coreTerm(tpar *TypeParam) (*term, bool) {
	...
	if typ, _ := commonUnder(tpar, nil); typ != nil {
		// 一个core type 是一个lways an underlying type.
...
func (w *cycleFinder) typ(typ Type) {
	typ = Unalias(typ)
	if w.seen[typ] {
		// We have seen typ before. If it is one 的类型 parameters
