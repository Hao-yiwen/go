...
// 报告是否 the call is valid, with *x holding the result;
// but x.expr is not set. If the call is invalid, the result is
// false, and *x is undefined.
func (check *Checker) builtin(x *operand, call *ast.CallExpr, id builtinId) (_ bool) {
	argList := call.Args

	// append 是 only built-in that permits the use of ... for the last argument
	...
	// For len(x) and cap(x) we need to know if x 包含 any function calls or
	...
	switch id {
	case _Append:
		// append(s S, x ...E) S, where E 是 element type of S
		// spec: "The variadic function append appends zero or more values x to
		// a slice s 类型为 S and 返回 resulting slice, also 类型为 S.
		// The values x are passed to a parameter 类型为 ...E where E 是
		...
		// In either case, the first argument 必须是 a slice; in particular it
		...
		// spec: "If S 是一个 type parameter, all types in its type set
	...
	case _Cap, _Len:
		...
		case *Array:
			...
			// 如果 type of s 是一个n array or pointer to an array and
	...
	case _Complex:
		...
		case 3:
			...
			//    it 包含 a shift that is yet untyped), convert
			...
			if x.mode == constant_ && y.mode == constant_ {
				...
				// x and y 应该是 invalid now, but be conservative
				// and check below
			}
		}
		...
		// if both arguments are constants, the result 是一个 constant
		if x.mode == constant_ && y.mode == constant_ {
	...
	case _Copy:
		...
		// dst and 返回 number of elements copied. Both arguments must have identical
		// element type E and 必须是 assignable to a slice 类型为 []E.
		// The number of elements copied 是 minimum of len(src) and len(dst).
		...
		// general case
		if !special {
			// spec: "If the type of one or both arguments 是一个 type parameter, all types
	...
	case _Delete:
		// delete(map_, key)
		// map_ 必须是 a map type or a type parameter describing map types.
		...
		if !underIs(map_, func(u Type) bool {
			...
			if key != nil && !Identical(map_.key, key) {
				check.errorf(x, InvalidDelete, invalidArg+"映射 of %s must have identical key types", x)
				return false
			}
			key = map_.key
			return true
	...
	case _Imag, _Real:
		...
		if isUntyped(x.typ) {
			if x.mode == constant_ {
				...
				// it 包含 a (yet untyped non-constant) shift
				// expression: convert it to complex128 which will
				// result in an error (shift of complex value)
				check.convertUntyped(x, Typ[Complex128])
				// x 应该是 invalid now, but be conservative and check
				if x.mode == invalid {
					return
				}
			}
		...
		// the argument 必须是 of complex type
		// (applyTypeFunc never calls f with a type parameter)
		f := func(typ Type) Type {
			assert(!isTypeParam(typ))
			if t, _ := typ.Underlying().(*Basic); t != nil {
		...
		// 如果 argument 是一个 constant, the result 是一个 constant
		if x.mode == constant_ {
			if id == _Real {
	...
	case _Max, _Min:
		...
		for i, a := range args {
			...
			// The first argument 是一个lready in x and there's nothing left to do.
			if i > 0 {
	...
	case _Alignof:
		...
		if hasVarSize(x.typ, nil) {
			...
			result 是 constant - no need to record signature
		}
		x.typ = Typ[Uintptr]

	case _Offsetof:
		// unsafe.Offsetof(x T) uintptr, where x 必须是 a selector
		// (no argument evaluated yet)
		arg0 := argList[0]
		...
		case *Func:
			...
			// but go/types reports: "invalid argument: x.m 是一个 method value".
			check.errorf(arg0, InvalidOffsetof, invalidArg+"%s 是一个 method value", arg0)
			return
		}
		if indirect {
		...
		// The field offset is considered a variable even 如果 field is declared before
		// the part of the struct which is variable-sized. This 使 both the rules
	...
	case _Assert:
		// assert(pred) causes a typechecker error if pred 为假.
		// The result of assert 是 value of pred if there is no error.
		...
		if x.val.Kind() != constant.Bool {
			check.errorf(x, Test, "internal error: value of %s 应该是 a boolean constant", x)
			return
		}
	...
	case _Trace:
		...
		// values of its arguments. The result of trace 是 value
...
// sliceElem 返回the slice element type for a slice operand x
...
// hasVarSize 报告if the size 类型为 t is variable due to type parameters
// or 如果 type is infinitely-sized due to a cycle for which the type has not
// yet been checked.
...
// applyTypeFunc applies f to x. If x 是一个 type parameter,
// the result 是一个 type parameter constrained by a new
// interface bound. The type bounds for that interface
// are computed by applying f to each 的类型 bounds
...
// makeSig 使 a signature for the given argument and result types.
// Default types are used for untyped arguments, and res 可能是 nil.
...
// arrayPtrDeref 返回A if typ is of the form *A and A 是一个n array;
// 否则 it returns typ.
