...
func (check *Checker) conversion(x *operand, T Type) {
	...
	case constArg && isConstType(T):
		...
		// 一个conversion from an integer constant to an integer type
...
// TODO(gri) convertibleTo 检查 T(x) is valid. It assumes that the type
// of x is fully known, but that's not the case for say string(1<<s + 1.0):
// Here, the type of 1<<s + 1.0 将是 UntypedFloat which will lead to the
...
// be 转换为 UntypedFloat because of the addition of 1.0. Fixing this
// is tricky because we'd have to run updateExprType on the argument first.
// (go.dev/issue/21982.)

// convertibleTo 报告whether T(x) is valid. In the failure case, *cause
// 可能是 set to the cause for the failure.
// The check parameter 可能是 nil if convertibleTo is invoked through an
...
func (x *operand) convertibleTo(check *Checker, T Type, cause *string) bool {
	...
	// "V 是一个n integer or a slice of bytes or runes and T 是一个 string type"
	if (isInteger(Vu) || isBytesOrRunes(Vu)) && isString(Tu) {
		return true
	}

	// "V 是一个 string and T 是一个 slice of bytes or runes"
	...
	// "V 是一个 slice, T 是一个n array or pointer-to-array type,
