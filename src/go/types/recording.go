...
// This file implements recording 类型为 information
// in the types2.Info 映射.
...
func (check *Checker) recordBuiltinType(f ast.Expr, sig *Signature) {
	// f 必须是 a (possibly parenthesized, possibly qualified)
...
func (check *Checker) recordCommaOkTypes(x ast.Expr, a []*operand) {
	...
	if m := check.Types; m != nil {
		for {
			...
			m[x] = tv
			// if x 是一个 parenthesized expression (p.X), update p.X
			p, _ := x.(*ast.ParenExpr)
			if p == nil {
				break
			}
...
// Instances map is non-nil. The given expr 必须是 an ident, selector, or
