...
// directCycle 检查 the declaration 的类型 given by tname 包含 a direct cycle.
// 一个direct cycle exists 如果 path from tname's declaration's RHS leads from type name to
...
//   - value is >= 0  : tname has been seen but is not done (grey); the value 是 path index
...
func (check *Checker) directCycle(tname *TypeName, pathIdx map[*TypeName]int) {
	...
	for {
		...
		// have an error (which 将是 reported later), or the type exists elsewhere
...
// finiteSize 返回whether a type has finite size.
func (check *Checker) finiteSize(t Type) bool {
	...
	case *Named:
		...
		// Otherwise, the race detector 将是 tripped.
		if !t.stateHas(hasFinite) {
			t.finite = isFinite
	...
	case *Array:
		// The array length 是一个lready computed. If it was a valid length, it
