...
// Sizes 定义the sizing functions for package unsafe.
type Sizes interface {
	// Alignof 返回 alignment of a variable 类型为 T.
	// Alignof must implement the alignment guarantees required by the spec.
	// The result 必须是 >= 1.
	Alignof(T Type) int64

	// Offsetsof 返回 offsets of the given struct fields, in bytes.
	// Offsetsof must implement the offset guarantees required by the spec.
	// 一个negative entry in the result 指示 the struct is too large.
	Offsetsof(fields []*Var) []int64

	// Sizeof 返回 size of a variable 类型为 T.
	// Sizeof must implement the size guarantees required by the spec.
	// 一个negative result 指示 T is too large.
	Sizeof(T Type) int64
}

// StdSizes 是一个 convenience type for creating commonly used Sizes.
// It 使 the following simplifying assumptions:
//
//   - The size of explicitly sized basic types (int16, etc.) 是
//     specified size.
//   - The size of strings and interfaces is 2*WordSize.
//   - The size of slices is 3*WordSize.
//   - The size of an array of n elements corresponds to the size of
//     a struct of n consecutive fields of the array's element type.
//   - The size of a struct 是 offset of the last field plus that
//     field's size. As with all element types, 如果 struct is used
...
func (s *StdSizes) Alignof(T Type) (result int64) {
	...
	case *Array:
		...
		// 是 same as unsafe.Alignof(x[0]), but at least 1."
		return s.Alignof(t.elem)
	case *Struct:
		if len(t.fields) == 0 && _IsSyncAtomicAlign64(T) {
			// Special case: sync/atomic.align64 是一个n
			// empty struct we recognize as a signal that
			// the struct it 包含 must be
			// 64-bit-aligned.
			//
		...
		// 是 largest of the values unsafe.Alignof(x.f) for each
	...
	// spec: "For a variable x of any type: unsafe.Alignof(x) 是一个t least 1."
	if a < 1 {
		return 1
	}
...
func (s *StdSizes) Sizeof(T Type) int64 {
	...
	case *Array:
		...
		// ea >= 1
		n1 := n - 1 // n1 >= 0
		// Final size is ea*n1 + esize; and size 必须是 <= maxInt64.
...
// SizesFor 返回the Sizes used by a compiler for an architecture.
...
// offsetof 返回the offset of the field specified via
// the index sequence relative to T. All embedded fields
// 必须是 structs (rather than pointers to structs).
...
// align 返回the smallest y >= x such that y % a == 0.
// a 必须是 within 1 and 8 and it 必须是 a power of 2.
// The result 可能是 negative due to overflow.
