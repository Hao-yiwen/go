...
// 一个_TypeSet represents the type set of an interface.
// Because of existing language restrictions, methods can be "factored out"
// from the terms. The actual type set 是 intersection 的类型 set
// implied by the methods and the type set described by the terms and the
// comparable bit. To test whether a type is included in a type set
// ("implements" relation), the type must implement all methods _and_ be
// an element 的类型 set described by the terms and the comparable bit.
// If the term list 描述 the set of all types and comparable 为真,
// only comparable types are meant; in all other cases comparable 为假.
...
// IsEmpty 报告whether s 是 empty set.
func (s *_TypeSet) IsEmpty() bool { return s.terms.isEmpty() }

// IsAll 报告whether s 是 set of all types (corresponding to the empty interface).
func (s *_TypeSet) IsAll() bool { return s.IsMethodSet() && len(s.methods) == 0 }

// IsMethodSet 报告whether the interface t is fully described by its method set.
func (s *_TypeSet) IsMethodSet() bool { return !s.comparable && s.terms.isAll() }

// IsComparable 报告whether each type in the set is comparable.
...
// NumMethods 返回the number of methods available.
func (s *_TypeSet) NumMethods() int { return len(s.methods) }

// Method 返回the i'th method of s for 0 <= i < s.NumMethods().
// The methods are ordered by their unique ID.
func (s *_TypeSet) Method(i int) *Func { return s.methods[i] }

// LookupMethod 返回the index of and method with matching package and name, or (-1, nil).
...
// hasTerms 报告whether s has specific type terms.
func (s *_TypeSet) hasTerms() bool { return !s.terms.isEmpty() && !s.terms.isAll() }

// subsetOf 报告whether s1 ⊆ s2.
func (s1 *_TypeSet) subsetOf(s2 *_TypeSet) bool { return s1.terms.subsetOf(s2.terms) }

// all 报告whether f(t, u) 为真 for each (type/underlying type) pairs in s.
...
// is calls f with the specific type terms of s and 报告是否
...
// topTypeSet 可能是 used as type set for the empty interface.
var topTypeSet = _TypeSet{terms: allTermlist}

// computeInterfaceTypeSet 可能是 called with check == nil.
func computeInterfaceTypeSet(check *Checker, pos token.Pos, ityp *Interface) *_TypeSet {
	...
	// 一个infinitely expanding interface (due to a cycle) is detected
	// elsewhere (Checker.validType), so here we simply assume we only
	// have valid interfaces. Mark the interface as complete to avoid
	// infinite recursion 如果 validType check occurs later for some
	// reason.
	...
	// of a method I.m's Func Object of an interface I 是 same as that of
	...
	for i, typ := range ityp.embeddeds {
		...
		// The type set of an interface 是 intersection 的类型 sets of all its elements.
...
// invalidTypeSet 是一个 singleton type set to signal an invalid type set
// due to an error. It's also a valid empty type set, so consumers of
// type 设置may choose to ignore it.
var invalidTypeSet _TypeSet

// computeUnionTypeSet 可能是 called with check == nil.
// The result is &invalidTypeSet 如果 union overflows.
func computeUnionTypeSet(check *Checker, unionSets map[*Union]*_TypeSet, pos token.Pos, utyp *Union) *_TypeSet {
	...
	for _, t := range utyp.terms {
		...
		// The type set of a union expression 是 union
		// 的类型 sets of each term.
