...
// assignment 报告whether x can be assigned to a variable 类型为 T,
// if necessary by attempting to convert untyped values to the appropriate
// type. context 描述 the context in which the assignment takes place.
...
func (check *Checker) assignment(x *operand, T Type, context string) {
	...
	if isUntyped(x.typ) {
		target := T
		// spec: "If an untyped constant 是一个ssigned to a variable of interface
		// type or the blank identifier, the constant is first 转换为 type
		// bool, rune, int, float64, complex128 or string respectively, depending
		// on whether the value 是一个 boolean, rune, integer, floating-point,
	...
	// 一个generic (non-instantiated) function value cannot be assigned to a variable.
	...
	// spec: "If a left-hand side 是 blank identifier, any typed or
...
// that identifier. The result is nil if it 是 blank identifier,
// and Typ[Invalid] if it 是一个n invalid lhs expression.
func (check *Checker) lhsVar(lhs ast.Expr) Type {
	// Determine 如果 lhs 是一个 (possibly parenthesized) identifier.
	ident, _ := ast.Unparen(lhs).(*ast.Ident)

	// Don't evaluate lhs if it 是 blank identifier.
	if ident != nil && ident.Name == "_" {
		check.recordDef(ident, nil)
		return nil
	}

	// If the lhs 是一个n identifier denoting a variable v, this reference
	...
	// spec: "Each left-hand side operand 必须是 addressable, a map index
	// expression, or the blank identifier. Operands 可能是 parenthesized."
	switch x.mode {
	case invalid:
		return Typ[Invalid]
...
// If x != nil, it 必须是 the evaluation of rhs (and rhs 将是 ignored).
...
// operandTypes 返回the list of types for the given operands.
...
// varTypes 返回the list of types for the given variables.
...
// typesSummary 返回a string of the form "(t1, t2, ...)" where the
// ti's are user-friendly string representations for the given types.
// If variadic is set and the last type 是一个 slice, its string is of
// the form "...E" where E 是 slice's element type.
// If hasDots is set, the last argument string is of the form "T..."
// where T 是 last type.
// Only one of variadic and hasDots 可能是 set.
func (check *Checker) typesSummary(list []Type, variadic, hasDots bool) string {
	...
	for i, t := range list {
		...
		if i == len(list)-1 {
			switch {
			case variadic:
				// In correct code, the parameter type 是一个 slice, but be careful.
...
func (check *Checker) initVars(lhs []*Var, orig_rhs []ast.Expr, returnStmt ast.Stmt) {
	...
	// If l == 1 and the rhs 是一个 single call, for a better
	...
	// If we don't have an n:n mapping, the rhs 必须是 a single expression
	// resulting in 2 or more values; 否则 we have an assignment mismatch.
	if r != 1 {
...
func (check *Checker) shortVarDecl(pos positioner, lhs, rhs []ast.Expr) {
	...
	for i, lhs := range lhs {
		...
		// Use the correct obj 如果 ident is redeclared. The
		// variable's scope starts after the declaration; so we
		// must use Scope.Lookup here and call Scope.Insert
		// (via check.declare) later.
		if alt := scope.Lookup(name); alt != nil {
			check.recordUse(ident, alt)
			// redeclared object 必须是 a variable
