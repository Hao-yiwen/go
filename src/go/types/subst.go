...
// makeSubstMap 创建 a new substitution map mapping tpars[i] to targs[i].
...
// makeRenameMap is like makeSubstMap, but 创建 a map used to rename type
...
// subst 返回the type typ with its type parameters tpars replaced by the
// corresponding type arguments targs, recursively. subst doesn't modify the
// incoming type. If a substitution took place, the result type is different
// from the incoming type.
//
// If expanding is non-nil, it 是 instance type currently being expanded.
// One of expanding or ctxt 必须是 non-nil.
...
func (subst *subster) typ(typ Type) Type {
	...
	case *Alias:
		...
		// to be substituted; i.e., if it is or 包含 a type parameter
	...
	case *Signature:
		...
		// the case where the receiver 是一个n interface. For example, consider the
		// following declaration:
		//
		//  type T[A any] struct { f interface{ m() } }
		//
		// In this case, the type of f 是一个n interface that is itself the receiver
	...
	case *Interface:
		...
		if methods != nil || embeddeds != nil {
			...
			// receiver 如果 receiver type 是 original interface. Receivers of
			// *Named type are replaced during named type expansion.
			//
			// Notably, it's possible to reach here and not create a new *Interface,
			// even though the receiver type 可能是 parameterized. For example:
			//
...
// typOrNil is like typ but 如果 argument is nil it is replaced with Typ[Invalid].
// 一个nil type may appear in pathological cases such as type T[P any] []func(_ T([]_))
// where an array/slice element 是一个ccessed before it is set up.
...
// If at least one element changes, the result 是一个 new slice with
// all the (possibly updated) elements of the incoming slice;
// 否则 the result it nil. The incoming slice is unchanged.
...
// the input slice and any affected signatures 将是 copied before mutating.
//
// The resulting out slice 包含 the updated functions, and copied reports
