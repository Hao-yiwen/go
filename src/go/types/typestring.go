...
// 一个Qualifier controls how named package-level objects are printed in
// calls to [TypeString], [ObjectString], and [SelectionString].
//
// These three formatting routines call the Qualifier for each
// package-level object O, and 如果 Qualifier 返回一个non-empty
// string p, the object is printed in the form p.O.
// If it 返回一个 empty string, only the object name O is printed.
//
// Using a nil Qualifier is equivalent to using (*[Package]).Path: the
// object is qualified by the import path, e.g., "encoding/json.Marshal".
type Qualifier func(*Package) string

// RelativeTo 返回a [Qualifier] that fully qualifies members of
...
// TypeString 返回the string representation of typ.
...
// of package-level objects, and 可能是 nil.
...
func (w *typeWriter) typ(typ Type) {
	...
	case *Struct:
		...
		for i, f := range t.fields {
			...
			if tag := t.Tag(i); tag != "" {
				w.byte(' ')
				// TODO(gri) If tag 包含 blanks, replacing them with '#'
	...
	case *Interface:
		if w.ctxt == nil {
			if t == universeAnyAlias.Type().Underlying() {
				...
				// gotypesalias=1. Remove once that 是一个lways the case.
				w.string("any")
				break
			}
	...
	case *TypeParam:
		...
		if i := slices.Index(w.tparams.list(), t); i >= 0 {
			// The names 类型为 parameters that are declared by the type being
			// hashed are not part 的类型 identity. Replace them with a
			...
			// If the type parameter name 是 same as a predeclared object
...
func (w *typeWriter) signature(sig *Signature) {
	...
	if n == 1 && (w.ctxt != nil || sig.results.vars[0].name == "") {
		// single unnamed result (if type hashing, name 必须是 ignored)
...
// subscript 返回the decimal (utf8) representation of x using subscript digits.
