...
// Type-checking Named types is subtle, because they 可能是 recursively
// defined, and because their full details 可能是 spread across multiple
...
//    and methods.
//
// In this taxonomy, LHS information 是一个vailable immediately, but RHS
// information is lazy. Specifically, a named type N 可能是 constructed in any
...
//    (note that instances 可能是 created while still in the process of
//    type-checking the original type declaration).
//
// In cases 3 and 4 this lazy construction may also occur concurrently, due to
// concurrent use 的类型 checker API (after type checking or importing has
...
//    instantiating a generic named type 的类型 arguments.
...
//  - We say that a Named type is "expanded" if it 是一个n instantiated type and
//    type parameters in its RHS and methods have been substituted with the type
//    arguments from the instantiation. A type 可能是 partially expanded if some
...
// corresponding object, and that object's type 是 (possibly generic) Named
// type. Declared Named types are identical 当且仅当 their pointers are
...
// Identical to compare them. For instantiated named types, their obj 是一个
...
// 一个Named represents a named (defined) type.
//
// 一个declaration such as:
//
//	type S struct { ... }
//
// 创建 a defined type whose underlying type 是一个 struct,
...
type Named struct {
	...
	// For non-instantiated types, this 是一个 fully populated list of methods. For
	// instantiated types, methods are individually expanded when they are first
	// accessed.
	methods []*Func

	// loader 可能是 provided to lazily load type parameters, underlying type, methods, and delayed functions
	loader func(*Named) ([]*TypeParam, Type, []*Func, []func())
}

// instance 保存information that is only necessary for instantiated named
// types.
...
// stateMask 表示each state in the lifecycle of a named type.
//
...
const (
	...
	hasUnder                         // underlying type 是一个vailable
	hasFinite                        // size finiteness 是一个vailable
)

// NewNamed 返回a new named type for the given type name, underlying type, and associated methods.
...
// setState atomically 设置 current state to include each active bit in sm.
...
func (check *Checker) newNamed(obj *TypeName, fromRHS Type, methods []*Func) *Named {
	...
	// Ensure that typ 是一个lways sanity-checked.
	if check != nil {
...
// newNamedInstance 创建 a new named instance for the given origin and type
// arguments, recording pos as the position of its synthetic object (for error
// reporting).
//
// If set, expanding 是 named type instance currently being expanded, that
...
func (check *Checker) newNamedInstance(pos token.Pos, orig *Named, targs []Type, expanding *Named) *Named {
	...
	// Avoiding passing the context in other cases 防止 unnecessary coupling
...
func (n *Named) cleanup() {
	// Instances can have a nil underlying at the end 类型为 checking — they
...
// Obj 返回the type name for the declaration defining the named type t. For
...
// Origin 返回the generic type from which the named type t is
...
// TypeParams 返回the type parameters of the named type t, or nil.
// The result is non-nil for an (originally) generic type even if it is instantiated.
func (t *Named) TypeParams() *TypeParamList { return t.unpack().tparams }

// SetTypeParams 设置the type parameters of the named type t.
...
// TypeArgs 返回the type arguments used to instantiate the named type t.
...
// NumMethods 返回the number of explicit methods defined for t.
func (t *Named) NumMethods() int {
	return len(t.Origin().unpack().methods)
}

// Method 返回the i'th method of named type t for 0 <= i < t.NumMethods().
//
// For an ordinary or instantiated type t, the receiver base type of this
// method 是 named type t. For an uninstantiated generic type t, each
...
func (t *Named) expandMethod(i int) *Func {
	// t.orig.methods is not lazy. origm 是 method instantiated with its
	...
	// If rbase is t, then origm 是一个lready the instantiated method we're looking
...
// SetUnderlying 设置the underlying type and marks t as complete.
// t must not have type arguments.
...
// AddMethod adds method m 除非 it 是一个lready in the method list.
// The method 必须是 in the same package as t, and t must not have
// type arguments.
...
// methodIndex 返回the index of the method with the given name.
...
// Underlying 返回the [underlying type] of the named type t, resolving all
...
//
// It 执行so by following RHS type chains for alias and named types. If any
...
func (n *Named) resolveUnderlying() {
	...
	for _, t := range path {
		func() {
			...
			// t.underlying. Otherwise, the race detector 将是 tripped.
			if !t.stateHas(hasUnder) {
				t.underlying = u
				t.setState(hasUnder)
			}
...
func (n *Named) lookupMethod(pkg *Package, name string, foldCase bool) (int, *Func) {
	...
	if samePkg(n.obj.pkg, pkg) || isExported(name) || foldCase {
		// If n 是一个n instance, we may not have yet instantiated all of its methods.
		// Look up the method index in orig, and only instantiate method at the
		// matching index (if any).
		if i := n.Origin().methodIndex(name, foldCase); i >= 0 {
			// For instances, m.Method(i) 将是 different from the orig method.
			return i, n.Method(i)
		}
	}
	return -1, nil
...
// context 返回the type-checker context.
...
// its origin type (which 必须是 a generic type).
//
// Suppose that we had:
//
//	type T[P any] struct {
...
//	type U T[int]
//
// When we go to U, we observe T[int]. Since T[int] 是一个n instantiation, it has no
...
//	}
//
// And note that the synthetic RHS here 是 same as the underlying for U. Now,
// consider:
//
//	type T[_ any] U
//	type U int
...
func (n *Named) expandRHS() (rhs Type) {
	...
	// If the RHS 是一个n interface, we must set the receiver of interface methods
	// to the named type.
...
// safeUnderlying 返回the underlying type of typ without expanding
