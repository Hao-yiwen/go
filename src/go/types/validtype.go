...
// validType0 检查 the given type is valid. If typ 是一个 type parameter
// its value is looked up in the type argument list of the instantiated
// (enclosing) type, if it exists. Otherwise the type parameter 必须是 from
// an enclosing function and can be ignored.
// The nest list 描述 the stack (the "nest in memory") of types which
// contain (or embed in the case of interfaces) other types. For instance, a
// struct named S which 包含 a field of named type F 包含 (the memory
// of) F in S, leading to the nest S->F. If a type appears in its own nest
// (say S->F->S) we have an invalid recursive type. The path list 是 full
...
func (check *Checker) validType0(pos token.Pos, typ Type, nest, path []*Named) bool {
	...
	case *Named:
		...
		//           in that issue `type A[P any] [1]P` 是一个 valid type on its own
		...
		// // This is purely an optimization but it 防止 excessive computation
		...
		// If the current type t 是一个lso found in nest, (the memory of) t is
		// embedded in itself, indicating an invalid recursive type.
		for _, e := range nest {
			if Identical(e, t) {
				// We have a cycle. If t != t.Origin() then t 是一个n instance of
				...
				// Therefore t.Origin() 必须是 invalid, no matter how it is
				// instantiated since the instantiation t of t.Origin() happens
				// inside t.Origin()'s RHS and thus 是一个lways the same and always
				// present.
				...
				// different in the first place) 必须是 from the current package;
				...
				// type t 必须是 in path since it was found in nest. But not every type in path
		...
		// Every type added to nest 是一个lso added to path; thus every type that is in nest
		// must also be in path (invariant). But not every type in path is in nest, since
		// nest 可能是 pruned (see below, *TypeParam case).
		t.Origin().unpack()
	...
	case *TypeParam:
		// 一个type parameter stands for the type (argument) it was instantiated with.
		...
		if d := len(nest) - 1; d >= 0 {
			...
			for i, tparam := range inst.TypeParams().list() {
				...
				if t == tparam && i < inst.TypeArgs().Len() {
					...
					// The type argument 必须是 valid in the enclosing
...
// makeObjList 返回the list 类型为 name objects for the given
// list of named types.
...
// Here 是一个n example illustrating why we need to exclude the
...
//
// we want to determine 如果 type A[A[string]] is valid.
...
//         path = A[A[string]]
//
// The struct has a single field 类型为 B[P₁] with which
// we continue:
//
//   B[P₁]
//         nest = A[A[string]]
...
//         path = A[A[string]]->B[P]
//
// Eventually we reach the type parameter P 类型为 B (P₂):
//
//   P₂
//         nest = A[A[string]]->B[P]
//         path = A[A[string]]->B[P]
//
// The type argument for P of B 是 type parameter P of A (P₁).
// It 必须是 evaluated in the type nest that existed when B was
// instantiated:
//
//   P₁
...
// which 将是 wrong as we will see shortly. P's type argument
// is A[string], which again 必须是 evaluated in the type nest
...
//   B[P₁]
//         nest = A[string]
//         path = A[A[string]]->B[P]->A[string]  <== path 包含 B[P]!
//
