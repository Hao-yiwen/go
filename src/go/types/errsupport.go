...
// lookupError 返回a case-specific error when a lookup of selector sel in the
...
func (check *Checker) lookupError(typ Type, sel string, obj Object, structLit bool) string {
	// Provide more detail if there 是一个n unexported object, or one with different capitalization.
	// If selector and object are in the same package (==), export doesn't matter, 否则 (!=) it does.
...
// tail 返回the string s without its first (UTF-8) character.
